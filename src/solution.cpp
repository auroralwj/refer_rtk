...
static int sort_solstat(solstatbuf_t *statbuf)
{
	...
	//if(!(statbuf_data=new solstat_t [statbuf->n])){
    //if (!(statbuf_data=realloc(statbuf->data,sizeof(solstat_t)*statbuf->n))) {
	if (!(statbuf_data=(solstat_t*) malloc(sizeof(solstat_t)*statbuf->n))) {
...
{
    const int sys[]={SYS_GPS,SYS_GLO,SYS_GAL,SYS_QZS,SYS_SBS,SYS_CMP,0};
    ...
    const char *s7[]={"gps","glonass","galileo","qzss","sbas","bds","mix"};
    const char *s8[]={"OFF","CONT","INST","FIXHOLD","PPPAR","PPPAR_ILS","WLNL","TCAR","LAMBDA","FARA","SINGLE "};
