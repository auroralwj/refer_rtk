...
#define ARMODE_LAMBDA  8                   /* AR mode: LAMBDA */
#define ARMODE_FARA 9                      /* AR mode: continuous */
#define ARMODE_Single 10                   /* AR mode: Single epoch ar */
...
typedef struct {        /* observation data */
	...
	int num[4][NFREQ];    /* 四大系统每个系统观测数据个数 */
...
typedef struct                        /*double difference obs data*/
{
	gtime_t time;                     /*obs data time*/
	unsigned char rsat,usat;          /*ref/rov sat number*/
	unsigned char type,freq;          /*type(0:P 1:L) freq(0:f1 1:f2)*/
	double ddvalue;                   /*double difference value(cycle)*/
	int ddAmb;                        /*double difference ambiguity*/
	double ddRes;                     /*double difference residuals(m)*/
	double ddIONl;                     /*double difference ION*/
	double ddIONp;
	double ddTROP;                    /*double difference Troposphere delay*/
	int ddAmb0;                       /*直接结算出来的初始模糊度，也就是没加改正的模糊度*/
	int flag;                         
}ddobsd_t;
typedef struct
{
	int n,nmax;    /*double difference obs data number and maximum number*/
	ddobsd_t* data;/*double difference obs data*/
}ddobs_t;		 
...
typedef struct {        /* processing options type */
	...
	int modear;         /* /* 模糊度固定方法 lambda fara single AR mode (0:off,1:continuous,2:instantaneous,3:fix and hold,4:ppp-ar) */
	...
	int stomodel;       /* 随机模型 */
	double sigma;       /* 随机模型方差*/
	int solformat;      /* 结果输出类型(xyz&enu)*/
	int ambiguity;      /* 模糊度固定方法这个没用到，用的是modear */
	...
	/*Add by C.Zhang at 2014-5-4 15:44*/
	bool cflag;/*the flag of correction or not*/
	bool outflag;/*是否输出*/
} prcopt_t;
...
typedef struct {        /* file options type */
	...
	/*Added by JS.Zhang at 2013-8-5 20:19:30*/
	char ddrescorr[2][MAXSTRPATH];
} filopt_t;
...
typedef struct {        /* satellite status type */
	...
	double H1[NFREQ][3];/* 历史历元设计矩阵*/
	double H2[NFREQ][3];/* 当前历元设计矩阵*/
	double HH[NFREQ][3];/* 三差设计矩阵*/
    double resp[NFREQ]; /* residuals of pseudorange (m) */
    double resc[NFREQ]; /* residuals of carrier-phase (m) */
	double D1_resp[NFREQ];   /*双差伪距残差*/
	double D1_resc[NFREQ];   /*双差相位残差*/
	double D1_Stoc[NFREQ];   /*双差相位方差*/
	double D2_resp[NFREQ];   /*双差伪距残差*/
	double D2_resc[NFREQ];   /*双差相位残差*/
	double D2_Stoc[NFREQ];   /*双差相位方差*/
	double TD_resp[NFREQ];   /*三差伪距残差*/
	double TD_resc[NFREQ];   /*三差相位残差*/
	double TD_Stoc[NFREQ];   /*三差相位方差*/
	int Dd_num_L1;            /*双差数量*/
	int Dd_num_L2;            /*双差数量*/
	int Dd_num_P1;            /*双差数量*/
	int Dd_num_P2;            /*双差数量*/
	...
	int Td_num_P1;            /*三差数量*/
	int Td_num_P2;            /*三差数量*/
	int Zd_gnss[NFREQ];        /* 非差观测方程个数 */
	int Dd_gnss[NFREQ];        /* 双差观测方程个数 */ 
	int TD_gnss[NFREQ];        /* 三差观测方程个数 */
	double Zd_var[NFREQ];      /* 非差观测值方差（m^2） */
	double Dd_var[NFREQ];      /* 双差观测值方差（m^2） */
	double Td_var[NFREQ];      /* 三差观测值方差（m^2） */
	double Zd_resc[NFREQ];     /* 非差观测值常数项（m） */
	double Dd_resc[NFREQ];     /* 非差观测值常数项（m） */
	double Td_resc[NFREQ];     /* 非差观测值常数项（m） */
    unsigned char vsat[NFREQ]; /* valid satellite flag */
    unsigned char snr [NFREQ]; /* signal strength (0.25 dBHz) */
    unsigned char fix [NFREQ]; /* ambiguity fix flag (1:fix,2:float,3:hold) */
    unsigned char slip[NFREQ]; /* cycle-slip flag */
	unsigned int silpn[NFREQ]; /* Cycle-slip number */
    unsigned int lock [NFREQ]; /* lock counter of phase */
    unsigned int outc [NFREQ]; /* obs outage counter of phase */
	unsigned int slipN[NFREQ]; /* slip num (XG*CYJ)*/
	...
	double  pd[2][NFREQ];
	double lam[NFREQ];    /* 载波频率*/
	int refsatflag[NFREQ];/* 参考卫星标识*/
	int obsc[NFREQ];      /* 有效观测值个数 */
	int gross[NFREQ];     /* 粗差个数 */
	double alpha[NFREQ];  /* 粗差方差放大率 */												  
} ssat_t;
...
typedef struct {        /* RTK control/result type */
	...
	int GAL_P7;
	int GAL_P8;
	int dd_gnss[4][NFREQ];  /* 双差观测方程个数 */
	int Ref_Sat[4][NFREQ];
	int ns;       //共视卫星数
	int satprn[MAXSAT];  //共视卫星列表
	int solstat;         //上一历元解的状态 Modified by CYJ.2014/10/18
	double xc[3],Pc[3];     /* fixed states and their covariance */
	double dx_td[3];
	double N[9],W[3];//求三差解时用到的法方程和常数项
	double *Nx,*Wx;//zhangchao 2015.6.30 11:01
	double Sigma;
	double S;             /* S    =sum(Li'PiLi) */
	int sum_n;            /* sum_n=sum(ni) */
	double ru[3];/* 单历元计算模式的监测站近似坐标*/																
} rtk_t;
...
typedef struct         /*observation continuity result struct*/
{
	unsigned char sat; /*satellite number*/
	unsigned char freq;/*frequency number(1:L1 2:L2)*/
	unsigned char type;/*obs type(0:P 1:L 2:D 3:S)*/
	gtime_t ts;        /*start time*/
	gtime_t te;        /*end time*/
	unsigned ephnum;   /*epoch number*/
}obscontd_t;
typedef struct
{
	int n,nmax;      /*obs continuity result number and maximum number*/
	obscontd_t* data;/*obs continuity result data*/
}obscont_t;

typedef struct         /*cycle slip detect result struct*/
{
	gtime_t time;      /*cycle slip time*/
	unsigned char sat; /*cycle slip satellite number*/
	unsigned char freq;/*cycle slip frequency number*/
	int num;           /*cycle slip number*/
}cycleslipd_t;
typedef struct
{
	int n,nmax;        /*cycle slip result number and maximum number*/
	cycleslipd_t* data;/*cycle slip result data*/
}cycleslip_t;
/*Added by JS.Zhang at 2013-08-04 23:13:52*/
typedef struct                  /*single difference obs data*/
{
	gtime_t time;               /*obs data time*/
	unsigned char sat,rcvb,rcvr;/*satellite number/base station/rover station*/
	double sdL[NFREQ];          /*single difference carrier-phase (cycle) */
	double sdP[NFREQ];          /*single difference pseudorange*/
}sdobsd_t;
typedef struct
{
	int n,nmax;    /*single difference obs data number and maximum number*/
	sdobsd_t* data;/*single difference obs data*/
}sdobs_t;		 
/* global variables ----------------------------------------------------------*/
...
extern int inverse1(double *a, int n);									  
...
extern void comamb(int nv,long int nc,const nav_t *nav,double *amb,double *Lv,
	double *v,double* bias,int *vflg);
extern int searchamb(int nv,const nav_t *nav,double *v,double* bias,int *vflg,
	double *A,double *P);							   
...
extern int outrnxobsb(FILE *fp, const rnxopt_t *opt, const obsd_t *obs, int n,
                      int epflag);
extern void init_sta(sta_t *sta);
...
extern int Pre_Single(const obsd_t *obs, int n, const nav_t *nav,
	const prcopt_t *opt, sol_t *sol, double *azel,
	ssat_t *ssat, char *msg,bool base,double *p,double *p0,double *e);
extern double Adjust(int n,int t, double A[],double L[],double P[],
	double X[],double N[],double V[]);								   
/* precise positioning -------------------------------------------------------*/
extern void rtkinit(rtk_t *rtk, const prcopt_t *opt);
extern void rtkfree(rtk_t *rtk);
extern double pdop_Max(int ns, double *azel,int *list,int ipara);
extern int Pre_Single(const obsd_t *obs, int n, const nav_t *nav,
	const prcopt_t *opt, sol_t *sol, double *azel,ssat_t *ssat, char *msg,bool base,double *p,double *p0,double *e);																					  
...
extern int PreProcess(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav);
...
//extern void seach_slip(int nv,int nx,int *W,int *vflg,
//					   double *H,double *R,double *v,double *V,const nav_t *nav,double *ratio,
//					   double *rms_v,double *vpv,int *slp);
