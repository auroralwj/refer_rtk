...
/*函数名：   varerr() ---------------------------------------------------------
* 功能：     伪距测量误差方差
* 输入变量： prcopt_t *opt             处理选项
*            double el                 高度角
*            int sys                   卫星系统（GPS,GLONASS...）
* 输出值：   SQR(fact)*varr            伪距方差
*-----------------------------------------------------------------------------*/
                     ...
                     int iter, const prcopt_t *opt, double *var)
{
    ...
    *var=0.0;
	...
	//对于GPS/GLO/QZS进行L1-L2，对于GAL/SBS进行L1-L5
	...
	//如果没有P1-P2的码偏差，用TGD代替
    if (P1_P2==0.0&&(sys&(SYS_GPS|SYS_GAL|SYS_QZS))) {
        P1_P2=(1.0-gamma)*gettgd(obs->sat,nav);
    }
    if (opt->ionoopt==IONOOPT_IFLC) { /* dual-frequency */   //双频
        
        if (P1==0.0||P2==0.0) return 0.0;
        if (obs->code[i]==CODE_L1C) P1+=P1_C1; /* C1->P1 */
        if (obs->code[j]==CODE_L2C) P2+=P2_C2; /* C2->P2 */
        
        /* iono-free combination */      //消除电离层
        PC=(gamma*P1-P2)/(gamma-1.0);
    }
    else { /* single-frequency */     //单频
...
/*函数名：   ionocorr() ---------------------------------------------------------
* 功能：     电离层改正
* 输入变量： gtime_t time              时间
*            nav_t  *nav               导航数据
*            int    sat                卫星个数
*            double *pos               接收机的位置{经度，纬度，大地高}（弧度|米）
*            double *azel              方位角/高度角（弧度）
*			 int    ionoopt            电离层改正选项
*			 double *ion               电离层延迟
*            double *var               电离层延迟方差
* 输出值：   status                    状态（1：正常，0：错误）
*-----------------------------------------------------------------------------*/
...
*            double *rs                卫星发射时刻的位置
*			 double *dts               卫星钟差
*			 double *vare              位置和钟差的方差
*            int *svh                  卫星健康状态
*            nav_t *nav                导航文件
*            *x                        待估的位置分量
*            prcopt_t *opt             处理选项
*            double *v                 测量模型的改正数
*            double *H                 设计矩阵的转置（n x m）
*            double *var               电离层延迟方差
*            double *azel              方位角/高度角（弧度）
*            int *vsat                 有效卫星标志
*            double *resp              伪距残差
                   ...
                   double *resp, int *ns)
{
    ...
    ecef2pos(rr,pos);//将ECEF坐标系统中的位置坐标转换为大地坐标
    
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        vsat[i]=0; azel[i*2]=azel[1+i*2]=resp[i]=0.0;
        
        if (!(sys=satsys(obs[i].sat,NULL))) continue;//如果卫星导航系统为空，则终止本次循环
        ...
        if ((r=geodist(rs+i*6,rr,e))<=0.0||
            satazel(pos,e,azel+i*2)<opt->elmin) continue; //如果卫星的几何距离小于0或者高度角小于截止高度角则终止本次循环
                  ...
                  char *msg)
{
    ...
    dops(ns,azels,opt->elmin,dop);//DOP值
                  ...
                  double *resp, char *msg)
{
    ...
    for (i=0;i<3;i++) x[i]=sol->rr[i];//接收机的位置和速度
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if (norm(dx,NX)<1E-4) {   //计算n维向量的模
            sol->type=0;
            sol->time=timeadd(obs[0].time,-x[3]/CLIGHT);
            sol->dtr[0]=x[3]/CLIGHT; /* receiver clock bias (s) */   //接收机钟差
            sol->dtr[1]=x[4]/CLIGHT; /* glo-gps time offset (s) */   //GLONASS与GPS时间差
            sol->dtr[2]=x[5]/CLIGHT; /* gal-gps time offset (s) */   //Galileo与GPS时间差
            sol->dtr[3]=x[6]/CLIGHT; /* bds-gps time offset (s) */   //BeiDou与GPS时间差
            for (j=0;j<6;j++) sol->rr[j]=j<3?x[j]:0.0;
            for (j=0;j<3;j++) sol->qr[j]=(float)Q[j+j*NX];
            sol->qr[3]=(float)Q[1];    /* cov xy */     //xy的协因数
            sol->qr[4]=(float)Q[2+NX]; /* cov yz */     //yz的协因数
            sol->qr[5]=(float)Q[2];    /* cov zx */     //zx的协因数
                    ...
                    double *azel, int *vsat, double *resp, char *msg)
{
    ...
    if (stat) {
        time2str(obs[0].time,tstr,2); //将获得的时间转换为字符型格式（yyyy/mm/dd hh:mm:ss.ssss）
		satno2id(sat,name);     //将卫星数字转换为卫星的ID号
                  ...
                  const double *azel, const int *vsat, double *v, double *H)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        lam=nav->lam[obs[i].sat-1][0];//载波波长
        ...
        a[2]=sin(azel[1+i*2]);
        matmul("TN",3,1,3,1.0,E,a,0.0,e);//矩阵相乘
        
        /* satellite velocity relative to receiver in ecef */
		//在ECEF坐标系中卫星相对于接收机的速度
...
/*函数名：  pntpos() ----------------------------------------------------------
* 功能：    由单点定位计算接收机的位置，速度以及接收机钟差
* 输入变量：obsd_t *obs               观测文件
*           n                         观测文件的个数
*			nav_t *nav                导航文件
*			prcopt_t *opt             处理选项
*			sol_t *sol                解算
*			double *azel              方位角/高度角（弧度）（空值不输出）
*			ssat_t *ssat              卫星状态（空值不输出）
*			char    *msg              错误退出的消息
* 输出值：  状态（1表示正常，0表示错误）
* 说明：    假设Sbas、Galileo、Qzss以及Compass 与GPS系统的时间偏差以及接收机差别可以忽略（只涉及Glonass与GPS的时间偏差以及接收机偏差）
*-----------------------------------------------------------------------------*/
                  ...
                  char *msg)
{
    ...
    if (opt_.mode!=PMODE_SINGLE) { /* for precise positioning */     //不是单点定位
#if 0
        opt_.sateph =EPHOPT_BRDC;
#endif
        opt_.ionoopt=IONOOPT_BRDC;//电离层模型：广播模式
        opt_.tropopt=TROPOPT_SAAS;//对流层模型：Saastamoinen 模型
    }
    ...
    if (ssat) {
        for (i=0;i<MAXSAT;i++) {
            ...
            ssat[i].resp[0]=ssat[i].resc[0]=0.0;//伪距残差=载波相位残差=0.0
            ssat[i].snr[0]=0;
        }
        for (i=0;i<n;i++) {
	...
	//free()函数功能：释放malloc()函数给指针变量分配的内存空间的函数
