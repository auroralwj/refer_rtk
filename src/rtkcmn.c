...
extern void maticpy(double *A, const double *B, int n, int m)
{
    memcpy(A,B,sizeof(int)*n*m);
}
/* 按对角合并两个矩阵A(m1*n1),B(m2*n2),得到矩阵C((m1+m2)*(n1+n2)),即C=diag(A,B) 2013.12.2*/
extern void diag_pj(double *C, double *A, double *B,int m1,int n1,int m2,int n2)
{
	int i,j;
	int m=m1+m2;
	int n=n1+n2;
	
//	trace(2,"A:%3d*%3d\n",m1,n1);tracemat(2,A,m1,n1,10,4);
//	trace(2,"B:%3d*%3d\n",m1,n2);tracemat(2,B,m2,n2,10,4);

	for(i=0;i<n;i++)
		for(j=0;j<m;j++) {
			if(j<m1&&i<n1)       C[j+m*i]=A[j+m1*i];
			else if(j>=m1&&i<n1) C[j+m*i]=0.0;
			else if(j<m1&&i>=n1) C[j+m*i]=0.0;
			else                 C[j+m*i]=B[j-n1+m2*(i-m1)];
		}
//	trace(2,"C:%3d*%3d\n",m,n);tracemat(2,C,m,n,10,4);
}
/* 矩阵横向拼接C=[A,B],A(m1*n1),B(m1*n2),C(m1*(n1+n2)) */
extern void hx_pj(double *C, double *A, double *B, int m1, int n1, int n2)
{
	int i,j;
	int n=n1+n2;

//	trace(2,"A:%3d*%3d\n",m1,n1);tracemat(2,A,m1,n1,10,4);
//	trace(2,"B:%3d*%3d\n",m1,n2);tracemat(2,B,m1,n2,10,4);

	for(i=0;i<n;i++)
		for(j=0;j<m1;j++) {
			if(i<n1)       C[j+m1*i]=A[j+m1*i];
			else           C[j+m1*i]=B[j+m1*(i-n1)];
		}
//	trace(2,"C:%3d*%3d\n",m1,n);tracemat(2,C,m1,n,10,4);
}
/* matrix routines -----------------------------------------------------------*/
                  ...
                  const double *R, int n, int m)
{
	...
	/*-----------------------------------------------*/
	trace(2,"filter:  \n");
	trace(2,"x_   参数个数：%d:\n",k);tracemat(2,x_,1,k,13,4);
	trace(2,"P_   参数随机信息：%d:\n",k);tracemat(2,P_,k,k,9,4);
	trace(2,"H_   方程个数：%d:\n",m);tracemat(2,H_,k,m,11,6);
	trace(2,"R                :\n");  tracemat(2,R ,m,m,8,6);
	trace(2,"v    残差个数：%d:\n",m);tracemat(2,v ,1,m,13,6);
	/*----------------------------------------------*/
    info=filter_(x_,P_,H_,v,R,k,m,xp_,Pp_);
	trace(2,"滤波后结果xp_   参数个数：%d:\n",k);tracemat(2,xp_,1,k,13,3);
	trace(2,"Pp_   参数随机信息：%d:\n",k);tracemat(2,Pp_,k,k,9,4);
    for (i=0;i<k;i++) {
        x[ix[i]]=xp_[i];
...
extern void get_trace(const char *file)
{
	trace(3,"file=%s\n",file);
}
...
//	trace(2,"trph:%.3lf , trpw:%.3lf ,%.3lf %\n",trph,trpw,trph/(trpw+trph)*100);
    return trph+trpw;
}
