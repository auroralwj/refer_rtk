...
//Modified by XGAo,2014/3/12
const prcopt_t prcopt_default={ /* defaults processing options */
    PMODE_SINGLE,0,2,SYS_GPS,   /* mode,soltype,nf,navsys */
    15.0*D2R,{{0,0}},           /* elmin,snrmask */
    0,3,1,5,0,10,               /* sateph,modear,glomodear,maxout,minlock,minfix */
    0,0,0,0,                    /* estion,esttrop,dynamics,tidecorr */
    1,0,0,0,0,                  /* niter,codesmooth,intpref,sbascorr,sbassatsel */
	1,59,0,0,0.003,4,0,0,
    0,0,                        /* rovpos,refpos */
    {1000.0,1000.0,1000.0},          /* eratio[] *///Modified by XGAO
    ...
    5.0,30.0,30.0,             /* maxtdif,maxinno,maxgdop */
...
extern void satno2id(int sat, char *id)
{
    ...
    switch (satsys(sat,&prn)) {
        ...
        case SYS_CMP: sprintf(id," %02d",prn-MINPRNCMP+1); return;
...
*-----------------------------------------------------------------------------*/
static int filter0_(const double *x, const double *P, const double *H,
                   ...
                   double *xp, double *Pp)
{
	...
	//Modified by xGAO, 2014/10/20
	//加权Kalman
	//double tau=1;
	//double age_weight=exp(tau);
...
//单因子抗差矩阵
static int Robustfilter_(rtk_t *rtk,const double *x, const double *P, const double *H,
                   ...
                   double *xp, double *Pp)
{
	...
	//I: 残差序列提取
	int L_num=0;//相位观测值数量
	L_num=rtk->GPS_L1+rtk->GPS_L2+rtk->GLO_L1+rtk->GLO_L2+rtk->CMP_B1+rtk->CMP_B2;

	int P_num=0;//伪距观测值数量
	P_num=rtk->GPS_P1+rtk->GPS_P2+rtk->GLO_P1+rtk->GLO_P2+rtk->CMP_P1+rtk->CMP_P2;

	if(L_num>4 && P_num>4)
	{

		double *v_L =mat(L_num,1);
		double *vi_L=mat(L_num,1);
		double *v_P =mat(P_num,1);
		double *vi_P=mat(P_num,1);
		double *ei_L=mat(L_num,1);
		double *ei_P=mat(P_num,1);


		//1:GPS
		//GPS_L
		int gps_Lt     =rtk->GPS_L1+rtk->GPS_L2;
		int gps_Pt     =rtk->GPS_P1+rtk->GPS_P2;
		int gps_total  =gps_Lt+gps_Pt;
		for (int i=0;i<gps_Lt;i++)
			v_L[i]       =v[i]*0.001/R[i+i*m];
		for (int i=gps_Lt;i<gps_total;i++)
			v_P[i-gps_Lt]=v[i]/R[i+i*m];

		//2:GLO
		//GLO_L
		int glo_Lt     =rtk->GLO_L1+rtk->GLO_L2;
		int glo_Pt     =rtk->GLO_P1+rtk->GLO_P2;
		int glo_total  =glo_Lt+glo_Pt;
		for (int i=gps_total;i<gps_total+glo_Lt;i++)
			v_L[i-(gps_Pt)]       =v[i]*0.001/R[i+i*m];
		for (int i=(gps_total+glo_Lt);i<(gps_total+glo_total);i++)
			v_P[i-(glo_Lt+gps_Lt)]=v[i]/R[i+i*m];

		//3:CMP
		//CMP_L
		int cmp_Bt     =rtk->CMP_B1+rtk->CMP_B2;
		int cmp_Pt     =rtk->CMP_P1+rtk->CMP_P2;
		int cmp_total  =cmp_Bt+cmp_Pt;
		for(int i=(gps_total+glo_total);i<(gps_total+glo_total+cmp_Bt);i++)
			v_L[i-(gps_Pt+glo_Pt)]       =v[i]*0.001/R[i+i*m];
		for(int i=(gps_total+glo_total+cmp_Bt);i<(gps_total+glo_total+cmp_total);i++)
			v_P[i-(gps_Lt+glo_Lt+cmp_Bt)]=v[i]/R[i+i*m];


		//II: 中值计算
		matcpy(vi_L,v_L,L_num,1);
		matcpy(vi_P,v_P,P_num,1);
		//冒泡排序
		for (int ii=0;ii<L_num-1;ii++)
		{ 
			double m_temp;
			for(int jj=ii+1;jj<L_num;jj++)
			{
				if(vi_L[jj]<vi_L[ii])
				{ 
					m_temp   =vi_L[ii];
		...
		for (int ii=0;ii<P_num-1;ii++)
		{ 
			double m_temp;
			for(int jj=ii+1;jj<P_num;jj++)
			{
				if(vi_P[jj]<vi_P[ii])
				{ 
					m_temp   =vi_P[ii];
		...
		double L_mean=0.0;
		int L_m=int(L_num/2+0.5);
		//偶数
		if(L_num%2==0)
			L_mean=0.5*(fabs(vi_L[L_m])+fabs(vi_L[L_m+1]));
		else
			L_mean=fabs(vi_L[L_m]);

		double P_mean=0.0;
		int P_m=int(P_num/2+0.5);
		//偶数
		if(P_num%2==0)
			P_mean=0.5*(fabs(vi_P[P_m])+fabs(vi_P[P_m+1]));
		else
			P_mean=fabs(vi_P[P_m]);
		...
		////II: 中值计算
		//matcpy(vi_L,ei_L,L_num,1);
		//matcpy(vi_P,ei_P,P_num,1);
		////冒泡排序
		//for (int ii=0;ii<L_num-1;ii++)
		//{ 
		//	double m_temp;
		...
		L_mean*=1.483;
		P_mean*=1.483;
		trace(3,"Robustfilter: 残差v, 标准差,  中误差,   方差  ");
		//III: IGGIII模型

		double k1=2.5,k2=8.0;//阈值参数设定
		double thres=0.0;
		//仅修改对角线元素
		//L
		for(int i=0;i<L_num;i++)
		{
			v_L[i]/=L_mean;
			double val=(k2-fabs(v_L[i]))/(k2-k1);
			thres =k1*pow(val,2)/fabs(v_L[i]);
			thres =1.0/thres;
			...
			//GLO
			else if(i<(gps_Lt+glo_Lt))
			{
				int num_l=i+gps_Pt;
				if (fabs(v_L[i])<k1)
					continue;
				else if(fabs(v_L[i])>k2)
					R[num_l+num_l*m] =R[num_l+num_l*m]*10;
				else
					R[num_l+num_l*m]*=thres;
			}

			//CMP
			else
			{
				int num_l=i+gps_Pt+glo_Pt;
				if (fabs(v_L[i])<k1)
					continue;
				else if(fabs(v_L[i])>k2)
					R[num_l+num_l*m] =R[num_l+num_l*m]*10;
		...
		tracemat(3,v,m,1, 10, 3 );
		tracemat(3,v_L,L_num,1, 10, 3 );
		tracemat(3,R,m,m, 10, 3 );

		////P
		//for(int i=0;i<P_num;i++)
		//{
		//	v_P[i]/=P_mean;
		//	double val=(k2-fabs(v_P[i]))/(k2-k1);
		//	thres =k1*pow(val,2)/fabs(v_P[i]);
		//	thres =1.0/thres;
		...
		//		if (fabs(v_P[i])<k1)
		//			continue;
		//		else if(fabs(v_P[i])>k2)
		//			R[num_p+num_p*m] =R[num_p+num_p*m]*10;
		//		else
		//			R[num_p+num_p*m]*=thres;
		//	}

		//	//GLO
		//	else if(i<(gps_Pt+glo_Pt))
		//	{
		//		int num_p=i+gps_Lt+glo_Lt;
		//		if (fabs(v_P[i])<k1)
		//			continue;
		//		else if(fabs(v_P[i])>k2)
		//			R[num_p+num_p*m] =R[num_p+num_p*m]*10;
		...
		//	else
		//	{
		//		int num_p=i+gps_Lt+glo_Lt+cmp_Bt;
		//		if (fabs(v_P[i])<k1)
		//			continue;
		//		else if(fabs(v_P[i])>k2)
		//			R[num_p+num_p*m] =R[num_p+num_p*m]*10;
...
//双因子抗差矩阵
static int RobustDualfilter_(rtk_t *rtk,const double *x, const double *P, const double *H,
                   const double *v, double *R, int n, int m,
                   double *xp, double *Pp)
{
    double *F=mat(n,m),*Q=mat(m,m),*K=mat(n,m),*I=eye(n);
	...
	if(L_num>4 && P_num>4)
	{
		...
		int gps_total  =gps_Lt+gps_Pt;
		for (int i=0;i<gps_Lt;i++)
			v_L[i]       =v[i]/sqrt(R[i+i*m]);
		for (int i=gps_Lt;i<gps_total;i++)
			v_P[i-gps_Lt]=v[i]/sqrt(R[i+i*m]);
		...
		int glo_total  =glo_Lt+glo_Pt;
		for (int i=gps_total;i<gps_total+glo_Lt;i++)
			v_L[i-(gps_Pt)]       =v[i]/sqrt(R[i+i*m]);
		for (int i=(gps_total+glo_Lt);i<(gps_total+glo_total);i++)
			v_P[i-(glo_Lt+gps_Lt)]=v[i]/sqrt(R[i+i*m]);
		...
		int cmp_total  =cmp_Bt+cmp_Pt;
		for(int i=(gps_total+glo_total);i<(gps_total+glo_total+cmp_Bt);i++)
			v_L[i-(gps_Pt+glo_Pt)]       =v[i]/sqrt(R[i+i*m]);
		for(int i=(gps_total+glo_total+cmp_Bt);i<(gps_total+glo_total+cmp_total);i++)
			v_P[i-(gps_Lt+glo_Lt+cmp_Bt)]=v[i]/sqrt(R[i+i*m]);
		...
		//偶数
		if(L_num%2==0)
			L_mean=0.5*(vi_L[L_m]+vi_L[L_m+1]);
		else
			L_mean=vi_L[L_m];
		...
		//偶数
		if(P_num%2==0)
			P_mean=0.5*(vi_P[P_m]+vi_P[P_m+1]);
		else
			P_mean=vi_P[P_m];
		//杨元喜公式 
		L_mean/=0.6745;
		...
		double k1=1.5,k2=4.5;//阈值参数设定
		double thres=0.0;
		//自适应降权阵
		double *rij=new double [m*m];
		rij=zeros(m,m);
		//r[i,j]=r[j+i*m];对称矩阵
		...
		for(int i=0;i<L_num;i++)
		{
			...
			else if(i<(gps_Lt+glo_Lt))
			{
				...
				{
					rij[num_l+num_l*m] =10000;
				}
				else
				{
					rij[num_l+num_l*m] =thres;
				}
			}
		...
		for(int i=0;i<P_num;i++)
		{
			...
			if(i<gps_Pt)
			{
				...
				if (fabs(v_P[i])<k1)
					...
					rij[num_p+num_p*m] =10000;
				else
					rij[num_p+num_p*m] =thres;
			}
		...
		//利用得到的自适应降权因子计算双因子自适应降权因子
		//同时，重新计算协方差矩阵
		//1.GPS_L_P
		//for(int i=0;i<gps_Lt;i++)         //L
		//	for(int j=0;j<gps_Lt;j++)
		//	{
		//		rij[j+i*m]=rij[i+j*m]=sqrt(fabs(rij[i+i*m]*rij[j+j*m]));
		//		R[j+i*m] *= rij[j+i*m];
		//		R[i+j*m] *= rij[i+j*m];
		//	}
		//for(int i=gps_Lt;i<gps_total;i++) //P
		//	for (int j=gps_Lt;j<gps_total;j++)
		//	{
		//		rij[j+i*m]=rij[i+j*m]=sqrt(fabs(rij[i+i*m]*rij[j+j*m]));
		//		R[j+i*m] *= rij[j+i*m];
		//		R[i+j*m] *= rij[i+j*m];
		...
		//		//2.GLO_L_P
		//for(int i=gps_total;i<gps_total+glo_Lt;i++)           //L
		//	for(int j=gps_total;j<gps_total+glo_Lt;j++)
		//	{
		//		rij[j+i*m]=rij[i+j*m]=sqrt(fabs(rij[i+i*m]*rij[j+j*m]));
		//		R[j+i*m] *= rij[j+i*m];
		//		R[i+j*m] *= rij[i+j*m];
		//	}
		//for(int i=gps_total+glo_Lt;i<gps_total+glo_total;i++) //P
		//	for (int j=gps_total+glo_Lt;j<gps_total+glo_total;j++)
		//	{
		//		rij[j+i*m]=rij[i+j*m]=sqrt(fabs(rij[i+i*m]*rij[j+j*m]));
		//		R[j+i*m] *= rij[j+i*m];
		//		R[i+j*m] *= rij[i+j*m];
		...
		//				//3.CMP_L_P
		//for(int i=gps_total+glo_total;i<gps_total+glo_total+cmp_Bt;i++)             //L
		//	for(int j=gps_total+glo_total;j<gps_total+glo_total+cmp_Bt;j++)
		//	{
		//		rij[j+i*m]=rij[i+j*m]=sqrt(fabs(rij[i+i*m]*rij[j+j*m]));
		//		R[j+i*m] *= rij[j+i*m];
		//		R[i+j*m] *= rij[i+j*m];
		//	}
		//for(int i=gps_total+glo_total+cmp_Bt;i<gps_total+glo_total+cmp_total;i++)   //P
		//	for(int j=gps_total+glo_total+cmp_Bt;j<gps_total+glo_total+cmp_total;j++)
		//	{
		//		rij[j+i*m]=rij[i+j*m]=sqrt(fabs(rij[i+i*m]*rij[j+j*m]));
		//		R[j+i*m] *= rij[j+i*m];
		//		R[i+j*m] *= rij[i+j*m];
		...
		//1.GPS_L_P
		//for(int i=0;i<gps_total;i++)         //L
		//	for(int j=0;j<gps_total;j++)
		//	{
		//		rij[j+i*m]=rij[i+j*m]=sqrt(fabs(rij[i+i*m]*rij[j+j*m]));
		//		R[j+i*m] *= rij[j+i*m];
		//		R[i+j*m] *= rij[i+j*m];
		...
		//			//		//2.GLO_L_P
		//for(int i=gps_total;i<gps_total+glo_total;i++)           //L
		//	for(int j=gps_total;j<gps_total+glo_total;j++)
		//	{
		//		rij[j+i*m]=rij[i+j*m]=sqrt(fabs(rij[i+i*m]*rij[j+j*m]));
		//		R[j+i*m] *= rij[j+i*m];
		//		R[i+j*m] *= rij[i+j*m];
	 ...
	 //   		//				//3.CMP_L_P
		//for(int i=gps_total+glo_total;i<gps_total+glo_total+cmp_total;i++)             //L
		//	for(int j=gps_total+glo_total;j<gps_total+glo_total+cmp_total;j++)
		//	{
		//		rij[j+i*m]=rij[i+j*m]=sqrt(fabs(rij[i+i*m]*rij[j+j*m]));
		//		R[j+i*m] *= rij[j+i*m];
		//		R[i+j*m] *= rij[i+j*m];
...
//抗差EKF
extern int Robustfilter(rtk_t *rtk, double *x, double *P, const double *H, const double *v,
                  double *R, int n, int m)
{
	...
	//info=filter_(x_,P_,H_,v,R,k,m,xp_,Pp_);
	//单因子抗差模型
    info=Robustfilter_(rtk,x_,P_,H_,v,R,k,m,xp_,Pp_);
	//双因子抗差模型
	//info=RobustDualfilter_(rtk,x_,P_,H_,v,R,k,m,xp_,Pp_);
...
/* Mean*/
//计算数据均值
extern double Mean(double *x,int n)
{
	double sum_x=0.0;
	for (int i=0;i<n;i++)
		sum_x+=x[i];
...
/* Std*/
//计算均方根误差
extern double Std(double *x,int n)
{
	double mean_x=Mean(x,n);
	double sum_x=0.0;
	for(int i=0;i<n;i++)
		sum_x=sum_x+(x[i]-mean_x)*(x[i]-mean_x);
	sum_x/=n;
	return sqrt(sum_x);
}
...
//Modified by CYJ
// 求x的n次方
extern double exn(double x, int n)
{
	int i;
	double xn=1.0;
//	trace(3,"求%10.3lf的%d次方\n",x,n);
	for(i=0;i<n;i++) xn*=x;
	return xn;
}

extern void maticpy(int *A, const int *B, int n, int m)
{
    memcpy(A,B,sizeof(int)*n*m);
}

//输出整数
extern void matifprint(const int A[], int n, int m, int p,  FILE *fp)
{
    int i,j;
...
extern void tracemati(int level, const int *A, int n, int m, int p)
{
    if (!fp_trace||level>level_trace) return;
    matifprint(A,n,m,p,fp_trace); fflush(fp_trace);
}
