...
/* single-differenced code-phase linear combination of phase --------------*/
static double cpobs(const obsd_t *obs, int i, int j,int f, const double *lam)
{
	double pi=sdobs(obs,i,j,f+NFREQ),pj=sdobs(obs,i,j,f)*lam[f];
	return pi==0.0||pj==0.0?0.0:pi-pj;
}
	...
	const nav_t *nav)
{
	...
	trace(2,"time:%s  g1-g0=%10.3lf\n",time_str(rtk->sol.time,3),g1-g0);
...
/* detect cycle slip by geometry free phase jump -----------------------------*/
static void detslp_code_ph(rtk_t *rtk, const obsd_t *obs, int i, int j,int f,
	const nav_t *nav)
{
	int sat=obs[i].sat;
	double g0,g1;
	double thresslip=8;    //伪距-载波探测周跳，周跳阀值8周

	trace(3,"detslp_code_ph: i=%d j=%d,freq=%d\n",i,j,f);

	if ((g1=cpobs(obs,i,j,f,nav->lam[sat-1]))==0.0) return;

	g0=rtk->ssat[sat-1].cp; rtk->ssat[sat-1].cp=g1;

	trace(2,"time:%s  cp1-cp0=%10.3lf\n",time_str(rtk->sol.time,3),g1-g0);
	trace(2,"slip detected (sat=%2d cp=%.3f %.3f)\n",sat,g0,g1);

	if (g0!=0.0&&fabs(g1-g0)>thresslip*nav->lam[sat-1][f]) {

		rtk->ssat[sat-1].slip[f]|=1;

		errmsg(rtk,"slip detected (sat=%2d cp=%.3f %.3f)\n",sat,g0,g1);
	}
}
...
/* detect cycle slip by code-phase-------20140930--------------------------------*/
extern void detslp_code_phase(rtk_t *rtk, const obsd_t *obs, const int *sat,
	const int *iu, const int *ir, int ns, const nav_t *nav,const double *azel)
{
	int i,f;

	trace(3,"detslp_code_phase:\n");
	...
	for (i=0;i<ns;i++) {
		...
		for (f=0;f<rtk->opt.nf;f++) 
			...
			detslp_code_ph(rtk,obs,iu[i],ir[i],f,nav);    //站际差分伪距减载波探测大的周跳 
	}
	...
	const int *iu, const int *ir, int ns, const nav_t *nav)
{
	...
	int i,j,f,reset;
	int now_slipflg[MAXX];
	trace(3,"udbias  : tt=%.1f ns=%d\n",tt,ns);

	get_now_slipflg(now_slipflg);   //将站际历元二次差探测的周跳情况读取出来
	...
	for (i=0;i<ns;i++) {
		...
		/* 采用伪距-载波相位组合探测周跳（主要是为后面snooping_single减少计算） */
		//     detslp_code_ph(rtk,obs,iu[i],ir[i],f,nav);

		/* detect cycle slip by doppler and phase difference */
		detslp_dop(rtk,obs,iu[i],1,nav);
		detslp_dop(rtk,obs,ir[i],2,nav);

		/* 由站际历元二次差分模型探测与修复的周跳情况 */
		for (f=0;f<rtk->opt.nf;f++) {
	...
	double *azel)
{
	...
	double dtrp=0.0,dion=0.0,vtrp,vion,dtrp_h;
	...
	for (i=0;i<n;i++) {
		...
		dtrp_h=tropmapf(obs[i].time,pos,azel+i*2,NULL)*zhd;//干分量

		/*		   trace(2,"sat:%3d,映射求得的大气干分量zhd:%.3lf , zhd*mapf:%.3lf\n",
		obs[i].sat,zhd,dtrp_h);   */

		/* ionospheric corrections */
		/*	   if (!ionocorr(obs[i].time,nav,obs[i].sat,pos,azel+i*2,
		IONOOPT_BRDC,&dion,&vion)) {
		continue;
		}//电离层在50km以上区域，基线两端高差几乎没影响*/
		/* tropospheric corrections */
		if (!tropcorr(obs[i].time,nav,pos,azel+i*2,opt->tropopt,&dtrp,&vtrp)) {
			continue;
		} //航测数据需要进行大气改正(包括湿分量)，其他数据只需消除干分量即可
		//	   trace(2,"直接采用SAAS模型dion:%.3lf , dtrp:%.3lf\n",dion,dtrp);
		if(dtrp==0.0) {
			dtrp=dtrp_h;
		}
...
static int ddres_phase(rtk_t *rtk, const nav_t *nav, double dt, const double *x,
	...
	for (m=0;m<2;m++) /* for each system (0:gps/qzss/sbas,1:glonass) */
		...
		for (f=opt->nf;f<opt->nf*2;f++) {
			...
			for (j=0;j<ns;j++) {
				...
				rtk->ssat[sat[j]-1].resp[f-opt->nf]=v[nv];
...
static int	phase_dd_least(rtk_t *rtk,double *xp,double *H,double *R,double *v,int nv)
{
	double *dx,*x,*A,*E,*hi,*h,*atp,*apa,*apl;

	int i,j,k,*ix;

	int nx=rtk->nx;

	double lpl=0.0,*lp,m2=0.0,Dx[3];

	trace(2,"伪距差分定位：\n");

	ix=imat(nx,1);A=mat(nv,nx);E=eye(nx);lp=mat(1,nv);

	matmul("TN",nv,nx,nx,1.0,H,E,0.0,A);  //A=H';

	free(E);

	for(i=0,k=0;i<nx;i++) {
		hi=A+i*nv;
		if(norm(hi,nv)>0.0) ix[k++]=i;    //选取参与平差的参数
	}

	dx=mat(k,1);x=mat(k,1);atp=mat(k,nv);apa=mat(k,k);apl=mat(k,1);
	h=mat(k,nv);

	for(i=0;i<k;i++) {
		x[i]=xp[ix[i]];	
		for(j=0;j<nv;j++) h[i+j*k]=H[ix[i]+j*nx];
	}

	matmul("NN",k,nv,nv,1.0,h,R,0.0,atp);
	matmul("NT",k,k,nv,1.0,atp,h,0.0,apa);  //A'*P*A
	matmul("NN",k, 1,nv,1.0,atp,v,0.0,apl);  //A'*P*l

	if(!matinv(apa,k)){
		matmul("NN",k,1,k,1.0,apa,apl,0.0,dx);
		for(i=0;i<k;i++) x[i]+=dx[i];
	}

	trace(2,"伪距单点定位坐标值：\n");tracemat(2,xp,1,3,10,3);
	for(i=0;i<k;i++) xp[ix[i]]=x[i];

	trace(2,"伪距差分定位：h\n");	
	tracemat(2,h,k,nv,10,3);
	trace(2,"权阵R：\n");tracemat(2,R,nv,nv,10,4);
	trace(2,"v:\n"); tracemat(2,v,1,nv,10,3);
	trace(2,"dx:\t");tracemat(2,dx,1,k,10,3);
	trace(2,"伪距差分定位坐标值：\n");tracemat(2,xp,1,3,10,3);

	//计算验后单位权中误差

	matmul("TN",1,nv,nv, 1.0,  v, R,0.0, lp);      //v'pv=l'pl-(a'pl)'*dx
	matmul("NN",1, 1,nv, 1.0, lp, v,0.0,&lpl);
	matmul("TN",1, 1, k,-1.0,apl,dx,1.0,&lpl);
	m2=lpl/(nv-k);
	for(i=0;i<3;i++) Dx[i]=m2*apa[i+i*k];

	trace(2,"伪距差分验后单位权方差m2:%lf\n",m2);
	trace(2,"伪距差分坐标分量方差Dx\n");tracemat(2,Dx,1,3,10,4);

	//更新rtk->x:为了后面计算坐标固定解
	matcpy(rtk->x,xp,rtk->na,1);

	free(ix);free(dx);free(x);free(atp);
	free(apa);free(apl);free(lp);
...
static int resamb(rtk_t *rtk, double *bias, double *xa)
{
	...
	if (!(info=lambda(nb,2,y+na,Qb,b,s))) {
		...
		//	    trace(1,"time:%s,N(1)=",time_str(rtk->sol.time,3)); tracemat(1,b   ,1,nb,10,3); 
	...
	const nav_t *nav)
{
	...
	xp=zeros(rtk->nx,1); Pp=zeros(rtk->nx,rtk->nx); xa=mat(rtk->nx,1);

	dx=zeros(na,1); px=mat(6,1);//相邻历元坐标增量
