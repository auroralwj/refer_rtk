...
/* constants/macros ----------------------------------------------------------*/
#define MAXSLIP        210             //最大的周跳个数 Modified by CYJ 2014/10/13
#define SQR(x)      ((x)*(x))
...
static void outsolstat(rtk_t *rtk)
{
    ...
    for (i=0;i<MAXSAT;i++) {
        ...
        satno2id(i+1,id);
        for (j=0;j<nfreq;j++) {
            fprintf(fp_stat,"%d ,%.3f ,%s ,%d ,%.1f ,%.1f ,%.4f ,%.4f ,%d ,%.0f ,%d ,%d ,%d ,%d ,%d ,%d\n",
                    week,tow,id,j+1,ssat->azel[0]*R2D,ssat->azel[1]*R2D,
                    ssat->resp [j],ssat->resc[j]*1000,  ssat->vsat[j],ssat->snr[j]*0.25,
                  ...
                  const prcopt_t *opt, int *sat, int *iu, int *ir)
{
    ...
    for (i=0,j=nu;i<nu&&j<nu+nr;i++,j++) {
        ...
        else if (azel[1+j*2]>=opt->elmin) { /* elevation at base station 看的是基准站的高度角，不是两个站的*/
...
static void udpos(rtk_t *rtk, double tt)
{
    ...
    if (rtk->opt.mode==PMODE_STATIC) 
	{
		...
		//Modified by CYJ.2014/10/18
		if (norm(rtk->xc,3)<=0.0) return ;
		if (rtk->solstat!=1)
		{
			for (i=0;i<3;i++) initx(rtk,rtk->xc[i],rtk->Pc[i],i);
		}
		return;
	}
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
	...
	int now_slipflg[MAXSLIP]={0};//Modified by CYJ 2014/10/13
    
    trace(3,"udbias  : tt=%.1f ns=%d\n",tt,ns);

	//Modified by CYJ 2014/10/13
	//将站际历元二次差探测的周跳情况读取出来
	get_now_slipflg(now_slipflg);   
    
	//逐卫星探测周跳
    for (i=0;i<ns;i++) 
	{   
        /* detect cycle slip by LLI */
		//LLI 探测周跳（LLI: Loss of Lock Indicator,失锁标识符）
		//LLi范围为0-7. bit 0 置0则表示当前历元与前历元间可能发生周跳
        for (f=0;f<rtk->opt.nf;f++) 
		...
		// 无几何影响组合模型探测周跳，限于双频或三频数据，阈值设定为0.5周
		// GF: 忽略相邻历元间电离层残差影响，消去了与频率无关的几何距离与钟差影响
		// GF容易受到f1/f2=77/66类似周跳影响
        detslp_gf_L1L2(rtk,obs,iu[i],ir[i],nav);
        detslp_gf_L1L5(rtk,obs,iu[i],ir[i],nav);
        
        /* detect cycle slip by doppler and phase difference */
		// 由于钟跳问题多普勒周跳探测精度较低
		// 通过相邻历元间相位观测值差值与多普勒频移差值得到的残差序列判断周跳
        detslp_dop(rtk,obs,iu[i],1,nav);
        detslp_dop(rtk,obs,ir[i],2,nav);

		/* 由（站间差、历元差）二次差分模型探测与修复的周跳情况 */
		for (f=0;f<rtk->opt.nf;f++)
		{
			if(now_slipflg[IB(sat[i],f,&rtk->opt)]==1)
			{
				rtk->ssat[sat[i]-1].slip[f]|=1;   //周跳记录（由差分模型探测）
			}

			if(now_slipflg[IB(sat[i],f,&rtk->opt)]==2)
			{
				rtk->ssat[sat[i]-1].slip[f]&=0xFC;   //周跳修复或者虚报的周跳标记全部清零
			}
			trace(2,"sat=%3d,now_slipflg=%3d,rtk->ssat.slip[%2d]=%d\n",
				sat[i],now_slipflg[IB(sat[i],f,&rtk->opt)],f,rtk->ssat[sat[i]-1].slip[f]);
		}
    }
...
/* 更新非模糊度参数   20140730------------*/
static void udstate_FN(rtk_t *rtk, const obsd_t *obs, const int *sat,
	const int *iu, const int *ir, int ns, const nav_t *nav)
{
	double tt=fabs(rtk->tt),bl,dr[3];
...
/* 更新模糊度参数   20140730------------*/
static void udstate_N(rtk_t *rtk, const obsd_t *obs, const int *sat,
	const int *iu, const int *ir, int ns, const nav_t *nav)
{
	double tt=fabs(rtk->tt);

	trace(3,"udstate_N : ns=%d\n",ns);
...
// 函数：   int zdres();
// 功能：   组建非差方程
// 参数：   int base       %   base:1; rover:0;
//          obsd_t *obs    %   基准站或流动站观测数据
//          int n          %   观测数据个数s
//          double *rs     %   卫星空间坐标
//          double *dts    %   卫星钟差改正值
//          int *svh       %   卫星健康状况标识
...
//          double *rr     %   测站坐标
//          prcopt_t *opt  %   解算参数
//          int index      %   标识（base:1, rover:0）
//          double *y      %   观测方程常数项y=p0-p;
//          double *e      %   观测方程设计矩阵
//          double *azel   %   观测卫星方位角与高度角
// Return: 1(OK); 0(无测站坐标)
// XGAO, 2014/9
//////////////////////////////////////////////////////////////////////////////*/
...
//         double *azel  %   观测卫星方位角与高度角
//         int *iu       %   流动站卫星排列序号
//         int *ir       %   基准站卫星排列序号（ir=iu+nu）
//         int ns        %   共视卫星数
//         double *v     %   双差观测方程常数项
//         double *H     %   双差观测方程设计矩阵
//         double *R     %   双差观测方程方差阵
//         int *vflg     %   频率标识
// Return：  nv 双差观测值个数（P1+P2+L1+L2）
// XGAO, 2014/9
//////////////////////////////////////////////////////////////////////////////*/
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    Ri=mat(ns*nf*2+2,1);   //基准站单差观测方程方差阵
	Rj=mat(ns*nf*2+2,1);   //流动站单差观测方程方差阵
	im=mat(ns,1);          //电离层延迟因子
    tropu=mat(ns,1);       //流动站电离层延迟
	tropr=mat(ns,1);       //基准站对流层延迟
	dtdxu=mat(ns,3);       //流动站三维方向对流层延迟
	dtdxr=mat(ns,3);       //基准站三维方向对流层延迟
	...
	// 逐系统，逐频率，逐卫星组建双差观测方程
    for (m=0;m<4;m++)  for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++)//f0-f2(L1,L2,L3)
	{
        ...
        for (j=0;j<ns;j++) 
		{
            ...
            if (f<nf) 
			{
                if (opt->ionoopt!=IONOOPT_IFLC) //iono-free 模型
				{
            ...
            trace(2,"sat=%3d-%3d %s%d v=%13.3f R=%8.6f %8.6f\n",sat[i],
                  sat[j],f<nf?"L":"P",f%nf+1,v[nv],Ri[nv],Rj[nv]);//输出单差载波、P码误差等
...
//Modified by XGAO,2014/7/6
//双差残差值
static int ddres_resi(rtk_t *rtk, const nav_t *nav, double dt, const double *x,
	const double *P, const int *sat, double *y, double *e,
	double *azel, const int *iu, const int *ir, int ns, double *v,
	double *H, double *R, int *vflg)
{
	...
	double *tropr,*tropu,*dtdxr,*dtdxu,*Ri,*Rj,s,lami,lamj,fi,fj,df,*Hi=NULL;
	int i,j,k,m,f,ff,nv=0,nb[NFREQ*4*2+2]={0},b=0,sysi,sysj,nf=NF(opt);
	//Modified by XGao,2013/7
	//rtk->GPS_L1=rtk->GPS_L2=rtk->GPS_L5=rtk->GPS_P1=rtk->GPS_P2=rtk->GPS_P5=0;
	//rtk->GLO_L1=rtk->GLO_L2=rtk->GLO_P1=rtk->GLO_P2=0;
	//rtk->CMP_B1=rtk->CMP_B2=rtk->CMP_B3=rtk->CMP_P1=rtk->CMP_P2=rtk->CMP_P3=0;
	//rtk->GAL_L1=rtk->GAL_L5=rtk->GAL_L7=rtk->GAL_P1=rtk->GAL_P5=rtk->GAL_P7=0;
	...
	for (m=0;m<4;m++) /* m=0:gps/qzs/sbs,1:glo,2:gal,3:bds */
		...
		for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
			...
			for (j=0;j<ns;j++) {
				...
				//Modified by XGao,2013/9/3
				/*////////////////////////////////////Begin\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/
				double minAzi=0.0,maxAzi=0.0;
				double minEle=0.0,maxEle=0.0;

				//Mask1: azel[1]:[0,pi/4] && azel[0]:[pi/4,3pi/4];
				//minAzi=0.25*PI;
				//maxAzi=1.75*PI;
				//minEle=0;
				//maxEle=0.25*PI;
				//bool flag1=(fabs(azel[0+iu[j]*2])>0 && fabs(azel[0+iu[j]*2])<minAzi);
				//bool flag2=(fabs(azel[0+iu[j]*2])>maxAzi && fabs(azel[0+iu[j]*2])<(2*PI));
				//bool flag3=(fabs(azel[1+iu[j]*2])>minEle && fabs(azel[1+iu[j]*2])<maxEle);
				//if ((flag1 || flag2) && flag3)  continue;

				//Mask2: azel[1]:[0,pi/4] && azel[0]:[1.25*pi,1.75*pi];
				//minAzi=0.75*PI;
				//maxAzi=1.25*PI;
				//minEle=0;
				//maxEle=0.25*PI;
				//bool flag1=(fabs(azel[0+iu[j]*2])>minAzi && fabs(azel[0+iu[j]*2])<maxAzi);
				//bool flag2=(fabs(azel[1+iu[j]*2])>minEle && fabs(azel[1+iu[j]*2])<maxEle);
				//if (flag1 && flag2)  continue;

				//Mask3: azel[0]:[0,pi/4] && azel[0]:[1*pi,1.25*pi];
				//       azel[1]:[73/180*pi,pi/2]
				//bool flag1=(fabs(azel[0+iu[j]*2])>(0.25*PI) && fabs(azel[0+iu[j]*2])<(0.5*PI));
				//if (flag1) continue;
				//bool flag2=(fabs(azel[0+iu[j]*2])>(1.25*PI) && fabs(azel[0+iu[j]*2])<(1.5*PI));
				//if (flag2) continue;
				//bool flag3=(fabs(azel[1+iu[j]*2])>(73*PI/180) && fabs(azel[1+iu[j]*2])<(0.5*PI));
				//if (flag3) continue;
				/////////////////////////////////////End\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
				...
				//if(f==0)
				//{
				//	opt->err[1]=0.1074/1000;
				//	opt->err[2]=0.8333/1000;
				//}
				//if(f==1)
				//{		
				//	opt->err[1]=0.5194/1000;
				//	opt->err[2]=1.0838/1000;
				//}
				...
				//	{
				//		if(m==0)//GPS
				//		{	rtk->GPS_L1++;rtk->GPS_P1++;}
				//		if(m==1){
				//			rtk->GLO_L1++;rtk->GLO_P1++;}
				//		if(m==2){
				//			rtk->GAL_L1++;rtk->GAL_P1++;}
				//		if(m==3){
				//			rtk->CMP_B1++;rtk->CMP_P1++;}		
				//	}

				//	if(f==1)//L2(B2)
				//	{
				//		if(m==0)//GPS
				//		{	rtk->GPS_L2++;rtk->GPS_P2++;}
				//		if(m==1){
				//			rtk->GLO_L2++;rtk->GLO_P2++;}
				//		if(m==2){
				//			rtk->GAL_L5++;rtk->GAL_P5++;}
				//		if(m==3){
				//			rtk->CMP_B2++;rtk->CMP_P2++;}		
				//	}

				//	if(f==2)//L5(B3)
				//	{
				//		if(m==0)//GPS
				//		{	rtk->GPS_L5++;rtk->GPS_P5++;}
				//		if(m==2){
				//			rtk->GAL_L7++;rtk->GAL_P7++;}
				//		if(m==3){
				//			rtk->CMP_B3++;rtk->CMP_P3++;}		
				//	}
				//}
				/////////////////////////////////////////////////////////
...
//双差残差值
static int ddres_DD(rtk_t *rtk, const nav_t *nav, double dt, const double *x,
					  const double *P, const int *sat, double *y, double *e,
					  double *azel, const int *iu, const int *ir, int ns, double *v,
					  double *H, double *R, int *vflg,int tt)
{
	prcopt_t *opt=&rtk->opt;
	double bl,dr[3],posu[3],posr[3],didxi=0.0,didxj=0.0,*im;
	double *tropr,*tropu,*dtdxr,*dtdxu,*Ri,*Rj,s,lami,lamj,fi,fj,df,*Hi=NULL;
	...
	for (m=0;m<4;m++) /* m=0:gps/qzs/sbs,1:glo,2:gal,3:bds */
		...
		for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) 
		{
			...
			/////记录参考卫星/////////
			rtk->Ref_Sat[m][f0]=sat[i];
			if(f<nf)rtk->ssat[sat[i]-1].refsatflag[f]=1;
			...
			for (j=0;j<ns;j++) {
				...
				if (lami<=0.0||lamj<=0.0) continue;
				//Modified by XGAO,2014/7/20
				rtk->ssat[sat[j]-1].lam[ff]=lamj;
				rtk->ssat[sat[i]-1].lam[ff]=lami;
				...
				if (H) {
					for (k=0;k<3;k++) {
						...
						if(tt==1 && f<nf) rtk->ssat[sat[j]-1].H1[f][k]=Hi[k];
						if(tt==2 && f<nf) rtk->ssat[sat[j]-1].H2[f][k]=Hi[k];
					}
				}
				/* double-differenced ionospheric delay term */
				if (opt->ionoopt==IONOOPT_EST) {
				...
				if(tt==1 && f<nf)
				{
					rtk->ssat[sat[j]-1].D1_resc[f] = rtk->ssat[sat[j]-1].resc[f];
					rtk->ssat[sat[j]-1].D1_Stoc[f] = Ri[nv]+Rj[nv];
					rtk->ssat[sat[i]-1].D1_Stoc[f] = Ri[nv];
				}
				if(tt==2 && f<nf)
				{
					rtk->ssat[sat[j]-1].D2_resc[f] = rtk->ssat[sat[j]-1].resc[f];
					rtk->ssat[sat[j]-1].D2_Stoc[f] = Ri[nv]+Rj[nv];
					rtk->ssat[sat[i]-1].D2_Stoc[f] = Ri[nv];
				}
				///////////////////////////////////////////////////////////////
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa)
{
	...
	double *Qa,*Q2,*Q22; //Qa:非模糊度参数协方差阵； Q2:模糊度参数协方差阵（=Qb）
	...
	Qa=mat(na,na); Q2=mat(nb,nb);Q22=mat(nb,nb);
	...
	//Modified by XGAO
	//赋值
	//for(i=0;i<na;i++) for(j=0;j<na;j++)  Qa[i+j*na]=Qy[   i+(0+j)*ny];
	//matcpy(Q2,Qb,nb,nb);
	//if(!matinv(Qa,na) &&(!matinv(Q2,nb)))
	//{
	//	double *Qbaaa,*Qabbb,*Qbbbb; 
	//	Qbaaa = mat(nb,na);
	//	Qabbb = mat(na,nb);
	//	Qbbbb = eye(nb);

	//	matmul("TN",nb,na,na, 1.0,Qab ,Qa,0.0,Qbaaa);        //Qbbbb=inv(Q22)=inv(Q22)+Q21*inv(Q11)*Q12*inv(Q22)
	//	matmul("NN",na,nb,nb, 1.0,Qab ,Q2,0.0,Qabbb);
	//	matmul("NN",nb,nb,na, 1.0,Qbaaa ,Qabbb,1.0,Qbbbb);   //E+Qbbbb

	//	matmul("NN",nb,nb,nb, 1.0,Q2 ,Qbbbb,0.0,Q22);   //Q22=Q22*(E+Qbbbb)
	//	matinv(Q22,nb);
	//	free(Qbaaa);free(Qabbb);free(Qbbbb);
	//}

 //   if(norm(Q22,nb)<=0.0) matcpy(Q22,Qb,nb,nb);
	/////////////////////////////////////////////////////

    trace(2,"N(0)="); tracemat(2,y+na,1,nb,10,3);//单差浮点解
    
    /* lambda/mlambda integer least-square estimation */
    if (!(info=lambda(nb,2,y+na,Qb,b,s))) {
        
        trace(1,"N(1)="); tracemat(1,b   ,1,nb,10,3);//固定后的单差模糊度
        trace(4,"N(2)="); tracemat(4,b+nb,1,nb,10,3);//固定后的单差模糊度的协方差
...
/*#if 0*/ /* omitted v.2.4.0 */
    if (stat&&nv>NP(opt)) {
		...
		//Modified by XGAO,2014/10/17
		rtk->sol.sigma =SQRT(vv/nv)*1000;//Unit:mm
                  ...
                  const nav_t *nav)
{
    ...
    rs=mat(6,n);     //卫星位置与速度
	dts=mat(2,n);    //卫星钟差
	var=mat(1,n);    //卫星轨道方差（基于星历文件第6行第1个元素）
	y=mat(nf*2,n);   //观测方程常数项
	e=mat(3,n);      //设计矩阵
    azel=zeros(2,n); //卫星方位角[0]与高度角[1]
    
	//rtk->ssat初始化
    for (i=0;i<MAXSAT;i++)
	{
        rtk->ssat[i].sys=satsys(i+1,NULL);    //卫星系统
        for (j=0;j<NFREQ;j++) rtk->ssat[i].vsat[j]=rtk->ssat[i].snr[j]=0;
    }

    /* satellite positions/clocks */
	// 基于星历文件计算卫星坐标，根据钟差改正公式计算卫星钟差
    satposs(time,obs,n,nav,opt->sateph,rs,dts,var,svh);
    
    /* undifferenced residuals for base station */
	//组建基准站观测数据非差方程，得到设计矩阵e、常数项
    ...
    xp=mat(rtk->nx,1);           //待估参数序列
	Pp=zeros(rtk->nx,rtk->nx);   //待估参数方差
	xa=mat(rtk->nx,1);           //
    matcpy(xp,rtk->x,rtk->nx,1); //xp=rtk->x
    
    ny=ns*nf*2+2;                //ny=ns*nf*2 ??（伪距+载波）为嘛还要+2
    v=mat(ny,1);                 //双差方程常数项
	H=zeros(rtk->nx,ny);         //双差方程设计矩阵 
	R=mat(ny,ny);                //双差观测值方差
	bias=mat(rtk->nx,1);         //双差观测值改正值
    ...
    else if (stat!=SOLQ_NONE&&resamb_LAMBDA(rtk,bias,xa)>1)
	{
        ...
        if (zdres(0,obs,nu,rs,dts,svh,nav,xa,opt,0,y,e,azel))
		{
			...
			{
                /* hold integer ambiguity */
				// 当AR模型选为强约束模型时，若前期固定解个数大于minfix，则依据前期结果直接进行定位计算
	...
	// 保存解算结果
    if (stat==SOLQ_FIX)//固定解
	{
        for (i=0;i<3;i++) 
		{
            rtk->sol.rr[i]=rtk->xa[i];                 // 定位坐标
            rtk->sol.qr[i]=(float)rtk->Pa[i+i*rtk->na];// 定位结果方差
        }
        rtk->sol.qr[3]=(float)rtk->Pa[1];              // 定位结果协方差xy
        rtk->sol.qr[4]=(float)rtk->Pa[1+2*rtk->na];    //               yz
        rtk->sol.qr[5]=(float)rtk->Pa[2];              //               zx
    }
    else              //非固定解
	{
        for (i=0;i<3;i++) 
    ...
    for (i=0;i<MAXSAT;i++) for (j=0;j<nf;j++) 
	{
		...
		//Modified by XGAO+CYJ, 2014/10/17
    }
	// 内存释放
    free(rs); free(dts); free(var); free(y); free(e); free(azel);
...
// XGAO, 2014/9
//////////////////////////////////////////////////////////////////////////////*/
static int relpos_yjchen(rtk_t *rtk, const obsd_t *obs, int nu, int nr,
                  const nav_t *nav)
{
    prcopt_t *opt=&rtk->opt;
    gtime_t time=obs[0].time;
	...
	//dx:相邻历元坐标增量
	double *dx,*px;
	//na:坐标参数； nb:模糊度个数； nx:总参数个数
	int na=rtk->na,nb=rtk->nx-rtk->na,nx=rtk->nx ;
    ...
    if ((ns=selsat(obs,azel,nu,nr,opt,sat,iu,ir))<=0) 
	{
		...
		////更新非模糊度参数
	////Modified by CYJ
	udstate_FN(rtk,obs,sat,iu,ir,ns,nav);
	...
	//历元间差分信息计算以及周跳探测与修复
	info=dt2dx_one(rtk,nav,dt,xp,sat,ns,y,n,e,azel,iu,ir,dx,px);//周跳探测

	//更新模糊度参数
	udstate_N(rtk,obs,sat,iu,ir,ns,nav); 

	matcpy(xp,rtk->x,rtk->nx,1);  //更新模糊度参数初值
    ...
    for (i=0;i<niter;i++) 
	{
		...
		slip_repaire(rtk,nav,sat, ns,iu,y);
    ...
    /* save solution status */
	rtk->solstat=stat;    //更新解的状态 Modified by CYJ.2014/10/18
	if (stat==SOLQ_FIX)
	{
		if (rtk->sol.ratio>=30)
		{
			matcpy(rtk->xc,rtk->xa,1,3);
			for (i=0;i<3;i++)
			{
				rtk->Pc[i]=rtk->Pa[i+i*rtk->na]*100;
			}
		}
	}
    ...
    for (i=0;i<MAXSAT;i++) for (j=0;j<nf;j++) 
	{
        ...
        if (rtk->ssat[i].slip[j]&1) rtk->ssat[i].slipc[j]++;
		//Modified by XGAO+CYJ, 2014/10/17
		if (rtk->ssat[i].slipN[j] > 20) rtk->opt.exsats[i]=1; //卫星剔除
    }
	// 内存释放
    free(rs); free(dts); free(var); free(y); free(e); free(azel);
    free(xp); free(Pp);  free(xa);  free(v); free(H); free(R); free(bias);
...
/* relative positioning ------------------------------------------------------*/
// Modified by XGAO+CYJ,2014/10/13
static int relpos_CYJ(rtk_t *rtk, const obsd_t *obs, int nu, int nr, const nav_t *nav)
{
	prcopt_t *opt=&rtk->opt;
	gtime_t time=obs[0].time;
	double *rs,*dts,*var,*y,*e,*azel,*v,*H,*R,*xp,*Pp,*xa,*bias,dt;
	...
	dx=zeros(na,1);
	px=mat(6,1);//相邻历元坐标增量
	//????rtk->na=3,是坐标参数个数，是否应为（matcpy(xp,rtk->x,rtk->nx,1);）
	//matcpy(xp,rtk->x,rtk->na,1);  //分块最小二乘法计算(模糊度参数初值要固定)
	//////////////////////////////
	...
	/* 伪距差分定位 */
	/*	if ((nv=ddres_phase(rtk,nav,dt,xp,Pp,sat,y,e,azel,iu,ir,ns,v,H,R,vflg))<1) {
	errmsg(rtk,"no double-differenced residual\n");
	stat=SOLQ_NONE;
	}  
	// 伪距差分定位，最小二乘解算
	/*	phase_dd_least(rtk,xp,H,R,v,nv); 
	...
	//多普勒探测周跳
	 //detslp_doppler(rtk,obs, sat,iu, ir, ns, nav,azel);

	

	//detslp_code_phase(rtk,obs, sat,iu, ir, ns, nav,azel);

	//历元间差分信息计算以及周跳探测与修复
//	info=dt2dx_one(rtk,nav,dt,xp,sat,ns,y,n,e,azel,iu,ir,dx,px);
 ...
 //   udstate(rtk,obs,sat,iu,ir,ns,nav);
	...
	//trace(2,"x(0)="); tracemat(4,rtk->x,1,NR(opt),13,4);
	...
	for (i=0;i<niter;i++)
	{
		...
		matcpy(Pp,rtk->P,rtk->nx,rtk->nx);
		//Modified by XGAO, 2014/10/17
		if ((info=Robustfilter(rtk,xp,Pp,H,v,R,rtk->nx,nv))) {
			errmsg(rtk,"filter error (info=%d)\n",info);
			stat=SOLQ_NONE;
			break;
		}

		trace(2,"x(%d)=",i+1); tracemat(4,xp,1,NR(opt),13,4);
	}
	if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,svh,nav,xp,opt,0,y,e,azel)) {
...
//Modified by XGAO,2014/6/25
/* relative positioning ------------------------------------------------------*/
static int relpos_DD(rtk_t *rtk, const obsd_t *obs, int nu, int nr,
					 const nav_t *nav)
{
	prcopt_t *opt=&rtk->opt;
	gtime_t time=obs[0].time;
	...
	for (i=0;i<niter;i++) {
		...
		/* double-differenced residuals and partial derivatives */
		if ((nv=ddres_resi(rtk,nav,dt,xp,Pp,sat,y,e,azel,iu,ir,ns,v,H,R,vflg))<1) {
			errmsg(rtk,"no double-differenced residual\n");
			stat=SOLQ_NONE;
			break;
		}
	...
	if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,svh,nav,xp,opt,0,y,e,azel)) {
		...
		/* post-fit residuals for float solution */
		nv=ddres_resi(rtk,nav,dt,xp,Pp,sat,y,e,azel,iu,ir,ns,v,NULL,R,vflg);
	...
	/* resolve integer ambiguity by LAMBDA */
		...
		if (zdres(0,obs,nu,rs,dts,svh,nav,xa,opt,0,y,e,azel)) {
			...
			/* post-fit reisiduals for fixed solution */
			nv=ddres_resi(rtk,nav,dt,xa,NULL,sat,y,e,azel,iu,ir,ns,v,NULL,R,vflg);
	...
	//Modified by XGa0,2013,9
	//计算DOp值
	//if(info=filter_nolequ(rtk))
	//{
	//	rtk->sol.DOP=0.0;
	//	/*rtk->sol.fanshu=0.0;*/
	//}
...
//Modified by XGAO,2014/07/18
/* relative positioning ------------------------------------------------------*/
static int relpos_DDR(rtk_t *rtk, const obsd_t *obs, int nu, int nr,
					 const nav_t *nav,int tt)
{
	prcopt_t *opt=&rtk->opt;
	gtime_t time=obs[0].time;
	double *rs,*dts,*var,*y,*e,*azel,*v,*H,*R,*xp,*Pp,*xa,*bias,dt;
	...
	//记录共视卫星数
	rtk->ns=ns;
	for(i=0;i<ns;i++)rtk->satprn[i]=sat[i];
	//////////////////////////////////////
	...
	for (i=0;i<niter;i++) {
		...
		/* double-differenced residuals and partial derivatives */
		if ((nv=ddres_DD(rtk,nav,dt,xp,Pp,sat,y,e,azel,iu,ir,ns,v,H,R,vflg,tt))<1) {
			errmsg(rtk,"no double-differenced residual\n");
			stat=SOLQ_NONE;
			break;
		}
...
extern void rtkinit(rtk_t *rtk, const prcopt_t *opt)
{
    ...
    rtk->opt=*opt;
	for(i=0;i<3;i++)   //Modified by CYJ.2014/10/18
	{
		rtk->xc[i]=0.0;
		rtk->Pc[i]=0.0;
	}
...
//Modified by XGAO,2014/7/18
//构建双差观测方程
extern int rtkpos_DDR(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav, int tt)
{
	prcopt_t *opt=&rtk->opt;
	sol_t solb={{0}};
	gtime_t time;
	...
	if (opt->mode==PMODE_MOVEB) { /*  moving baseline */
		...
		if (fabs(rtk->sol.age)>opt->maxtdiff) {
			...
			outsolstat(rtk);
			return 0; //Modified by XGAO,2014/7/22
		}
	}
	/* relative potitioning */
	relpos_DDR(rtk,obs,nu,nr,nav,tt);
	return 1;
}
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    relpos_yjchen(rtk,obs,nu,nr,nav);
	//结果统计文件输出
    outsolstat(rtk);
    //返回
    return 1;
